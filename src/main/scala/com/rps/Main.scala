...
object Move {
  ...
  given Show[ReadError] = Show[String].contramap[ReadError] {
    case ReadError.InvalidMove(input) =>
...
final class CliPlayer[F[_]: Console: Monad](val name: String)
    extends Player[F] {
...
final class CliRockPaperScissors[F[_]: Console: Monad]
    extends RockPaperScissors[F] {

  override def round(player1: Player[F], player2: Player[F]): F[Result] = for {
    _         <- Console[F].println("The round has started")
    choiceOne <- player1.getMove()
    choiceTwo <- player2.getMove()
    _         <- Console[F].println(s"Player ${player1.name} choses $choiceOne")
    _         <- Console[F].println(s"Player ${player2.name} choses $choiceTwo")
...
object Main extends IOApp.Simple {
  ...
  def run: IO[Unit] = {
    ...
    val game    = new CliRockPaperScissors[IO]

    game.round(player1, player2).map(_.show) >>= IO.println
  }
}
